...
REM 1. Cap nhat file wokwi.toml
(
    echo [wokwi]
    echo version = 1
...
REM 2. Sao chep so do mach tuong ung sang diagram.json
if exist "wokwi\%BAI%\diagram.json" (
    copy /y "wokwi\%BAI%\diagram.json" "diagram.json" > nul
    if exist "..\lab2_iot" copy /y "wokwi\%BAI%\diagram.json" "..\diagram.json" > nul
...
if not exist ".pio\build\%BAI%\firmware.bin" (
    echo [*] Chua co firmware cho %BAI%. Dang tu dong bien dich bang PlatformIO...
    call "%USERPROFILE%\.platformio\penv\Scripts\pio.exe" run -e %BAI%
) else (
...
echo   1. Mo file "diagram.json" trong VS Code (o thu muc goc lab2_iot).
echo   2. Nhan nut PLAY (tam giac mau xanh la cay) de CHAY MO PHONG.
echo   3. Mo tab "Wokwi" hoac xem Serial Monitor (115200 baud) ngay ben duoi.
echo ====================================================================
echo.
